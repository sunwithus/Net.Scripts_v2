...
namespace MudBlazorWeb2.Components.Modules.ProcessingDB
{
    ...
    {
        /*public List<string> _ignoreRecordType { get; set; }
        public EFCoreLinq(IConfiguration configuration) 
        {
            //_codecs = configuration.GetSection("EncodingDecoding:Codecs").Get<List<string>>();
            _ignoreRecordType = IConfiguration.GetSection("EncodingDecoding:IgnoreRecordTypes").Get<List<string>>();
        }*/
        ...
        public static List<SPR_SPEECH_TABLE> GetSpeechRecords(DateTime startDate, DateTime endDate, OracleDbContext db, List<string> _ignoreRecordType)
        {
            ...
            return db.SprSpeechTable.Where(x => x.Datetime >= startDate && x.Datetime <= endDate)
                ...
                .Where (x => !_ignoreRecordType.Contains(x.Eventcode))
